...
void BOOT_Init(void)
{
 GPIO_InitTypeDef GPIO_InitStructure;
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;	         		 		//USART1 TX         	 		//USART1 RX
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;   	 		//
  GPIO_Init(GPIOA, &GPIO_InitStructure);
...
void Main_Menu(void)
{
  ...
  while (1)
  {
       ...
       wait_time=80;
			 FlashEraseLastPage();
			 CanMode=1;
 			 Can_Send_Msg(canbuf,8);//·¢ËÍ8¸ö×Ö½Ú 
			 delay_ms(50);
			 Can_Send_Msg(&canbuf[8],8);
    }
    /* Download user application in the Flash */
		if(*((uint16_t*)BOOTSW)==1)
		{
    ...
    CanDownload();
		...
		RunMainProgram(ApplicationAddress);
  }
}
...
void RunMainProgram(uint32_t ProgramAddr) {
	if (((*(__IO uint32_t*)ProgramAddr) & 0x2FFE0000 ) == 0x20000000) {
		SerialPutString("Load main program!\r\n");
		LCD_Printf(80, 140, " Load Main Program.");
		JumpAddress = *(__IO uint32_t*) (ProgramAddr + 4);
		...
		SerialPutString("Program not in FLASH!\r\n");
		LCD_Printf(74, 140, " Program not in FLASH!");
	}
}
